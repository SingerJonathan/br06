...
public class LongswordBleedMutation1 : Ability
{

    public int WeaponDamage = 1;
    public int VulnerableDamage = 1;
    public float VulnerableCooldown = 1f;
            ...
            if (_hitboxTriggerController.CollidingObjects[index].name.Contains(_enemyColour))
            {
                ...
                if (Physics.Raycast(origin.position, target.position - origin.position, out hit, 100, ~(1 << 8)))
                {
                    ...
                    if (hit.transform.name.Contains(_enemyColour))
                    {
                        ...
                        _hitboxTriggerController.CollidingObjects[index].GetComponent<CharacterStatsController>().VulnerableAttack(VulnerableCooldown, VulnerableDamage);
                    }
                }
            }
        }
